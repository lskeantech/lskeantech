...
#* 재민님이 삼성물산 재실 예측 해달라고 요청하심. 인풋은 아무거나 사용해도 되며, 점심을 주로 예측해달라고 하심(점심에 plug 줄어드는게 보이긴한다고 함)
...
f_name = current_dir  / '삼성물산_재실_예측' / 'data' / '12F_남서측.csv'
...
df['heatcool'] = df['heating_use'] + df['cooling_use']

# plug 변화량 추가
df['plug_change'] = df['plug'].diff().fillna(0)
...
end_date = datetime.datetime(year=2024, month=11, day=19, hour=23, minute=45)
...
def is_working_day_and_time(x):
    ...
    if x.weekday() >= 5 or x in kr_holidays:
        return 0
    elif 8 <= x.hour <= 18:  # 근무 시간 9:00~18:00
        # 점심시간 (11:30~13:00) 동안 0을 반환
        if x.hour == 11 and x.minute >= 30:  # 11:30~11:59
            return 0
        elif x.hour == 12:  # 12:00~12:59
            return 0
        elif x.hour == 13 and x.minute < 1:  # 13:00~13:00:59
            return 0
        return 1
    else:
        return 0
...
for target_date in date_range:
  ...
  # hmm.fit(data_20[['plug','weekday','Co2','plug_change','Co2_change']])
  hmm.fit(data_20[['plug','Co2','weekday']])
  # plug_today = target_data.loc[target_start:target_end, ['plug_weekday','timedata_weekday','Co2','plug_mean_5']]
  # plug_today = target_data.loc[target_start:target_end, ['plug','weekday','Co2','plug_change','Co2_change']]
  plug_today = target_data.loc[target_start:target_end, ['plug','Co2','weekday']]
 ...
 # ! 재복님이 이부분 추가해줘서 갑자기 정확도가 확 좋아졌음. 결측치 있는 경우, 예측하지 않는 내용이라고 함. 
 ...
 # !   
  ...
  daily_energy_waste.loc[target_start, 'Co2'] = data_1d.Co2.sum()
...
# ! 재복님이 co2 그래프도 그려보는게 어떨까 얘기함
# 5. Co2
fig.add_trace(go.Scatter(x=target_data.index, y=target_data.Co2, mode='lines', name='냉난방사용량'), row=5, col=1)
...
fig.update_layout(
    title="재실 여부 및 에너지 사용량 분석",
    title_x=0.5,  # 제목을 중앙에 정렬
    title_y=0.95,  # 제목 위치 조정
    title_font=dict(size=20)  # 제목 폰트 크기 설정
)
