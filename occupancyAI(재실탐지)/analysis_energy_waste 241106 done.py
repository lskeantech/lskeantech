# 기본 베이스 자료에서 여기까지 옴. 이게 11.07 해석까지 완료한 자료임. 여기서 이것저것 테스트해보고싶다면, 복사본 만들어서 진행할 것

import pandas as pd
import datetime
import plotly.graph_objects as go
from hmmlearn.hmm import GaussianHMM
f_name = r"C:\Users\User\Desktop\승건\데사팀\occupancyAI(재실탐지)\ean_energy\csv\merged_7f.csv"
...
start_date = datetime.datetime(year=2022, month=12, day=1, hour=0, minute=0)
end_date = datetime.datetime(year=2023, month=11, day=30, hour=23, minute=59)
...
for target_date in date_range:
  ...
  date_start_2w = target_start - datetime.timedelta(days=14)
  ...
  hmm.fit(data_2w['plug'].values.reshape(-1, 1))
